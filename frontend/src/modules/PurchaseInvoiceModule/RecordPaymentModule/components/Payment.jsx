...
export default function PurchasePayment({ config, currentItem }) {
    ...
    <>
      <Row gutter={[12, 12]}>
        ...
        >
          <PageHeader
            ...
            title={`Record PurchasePayment for ${ENTITY_NAME} # ${currentErp.number}/${
              currentErp.year || ''
            }`}
            ghost={false}
            ...
            extra={[
              ...
              <Button
                key={`${uniqueId()}`}
                onClick={() => navigate(`/purchaseInvoice/read/${currentErp._id}`)}
                icon={<FileTextOutlined />}
              >
                {translate('Show PurchaseInvoice')}
              </Button>,
            ]}
            style={{
              padding: '20px 0px',
      ...
      <Row gutter={[12, 12]}>
        ...
        >
          ...
          <Descriptions title={`${translate('Supplier')}  : ${currentErp.supplier.name}`} column={1}>
